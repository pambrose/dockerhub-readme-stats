...
- **`fetchLatestVersion` resolves the version by digest, not by picking the highest number.** The naive "highest semver tag" approach is wrong in ways that are easy to miss: postgres tags releases as two-part (`18.4`) and its `19beta2` prerelease would win, and repos that never tagged a version get one invented for them. Matching the digest of `:latest` answers the narrower question — which version tag is an alias of what `:latest` serves. It returns `null` rather than throwing, because the version is decorative and a card that renders without it beats a card that 500s. Verified against nginx, postgres, redis, traefik, grafana, prometheus, vault, ubuntu, alpine, and mysql.
